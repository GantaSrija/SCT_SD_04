...
def scrape_walmart_products(search_query, max_products=5):
    ...
    try:
        ...
        for item in items:
            try:
                name = item.select_one('[data-automation-id="product-title"]').get_text(strip=True)
                price = item.select_one('[data-automation-id="product-price"]').get_text(strip=True)
...
def save_to_csv(data, filename):
    ...
    with open(filename, 'w', newline='', encoding='utf-8') as file:
        writer = csv.DictWriter(file, fieldnames=['Name', 'Price', 'Rating', 'Source'])
        writer.writeheader()
    ...
    if not search_term:
        ...
        if combined_products:
            ...
            for idx, product in enumerate(combined_products, 1):
                print(f"{idx}. [{product['Source']}] {product['Name']} | {product['Price']} | Rating: {product['Rating']}")
